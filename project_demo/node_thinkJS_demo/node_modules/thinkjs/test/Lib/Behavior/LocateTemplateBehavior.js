...
describe('LocateTemplateBehavior', function(){
  ...
  it('empty file', function(done){
    getTestPromise('').then(function(file){
      assert.equal(file.indexOf('View/Index/index_login.html') > -1, true);
      done();
    })
  })
  it('relative file', function(done){
    getTestPromise('Index/index_login.html').then(function(file){
      assert.equal(file.indexOf('View/Index/index_login.html') > -1, true);
      done();
    })
  })
  it('relative file', function(done){
    getTestPromise('/Index/index_login.html').then(function(file){
      assert.equal(file.indexOf('/Index/index_login.html') > -1, true);
      done();
    })
  })
